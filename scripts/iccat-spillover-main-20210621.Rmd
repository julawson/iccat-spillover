...
stockvolreg <- feols(Qty_t ~ tac_any | YearC^FleetGear + SpeciesStock, data = filter(iccat_df_final_edit, tacever==0))
...
stockreg <- feols(propStock ~ tac_any | YearC^SpeciesStock + FleetGear, data = filter(iccat_df_final_edit, tacever==0))
