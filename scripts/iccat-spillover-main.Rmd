...
iccat_df_final_edit <- iccat_df_final %>%
  ...
  right_join(filter, by="FleetGear")

```

```{r analaysis by stock}
#Question. Does the proportion of the catch of stocks that never receive a TAC increase or decrease relative to the catch of stocks that eventually recieve a TAC?
#Answer. There is no difference between the proportion of the catch of stocks that never receive a TAC and those that eventually receive one.

stockreg <- feols(propStock ~ tacever | YearC^SpeciesStock + FleetGear, data = filter(iccat_df_final_edit, tacr==0))
